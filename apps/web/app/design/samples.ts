...
export async function loadSampleCards(): Promise<Array<{ label: string; card: ShopCard }>> {
  const rows = await prisma.product.findMany({
    ...
    select: {
      id: true, sku: true, nameUk: true, nameRu: true, price: true, oldPrice: true, supplierAvailable: true, categoryId: true,
  ...
  const items: SearchItem[] = rows.map((r) => {
    ...
    return {
      ...
      available: r.supplierAvailable, image: r.images[0]?.url ?? null, categoryId: r.categoryId,
    };
  });
