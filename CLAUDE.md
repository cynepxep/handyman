...
| `pnpm test` | 159 проверок (core 110 + интеграционные db 49). Интеграционные идут на базе `handyman_test` и индексе `products_test` |
...
- **Фото товаров** — свои копии в `MEDIA_DIR` (`packages/db/src/media.ts`): везде, где читаются фото, брать `localUrl ?? url`. Картинки через `next/image` —
