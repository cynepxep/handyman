...
Проверка в браузере, когда чужой `next dev` на :3100 держит старый клиент базы и остановить его нельзя: `pnpm build` + `next start -p 3200` (своя копия), временный вход через `HM_TMP_LOGIN=1`.
